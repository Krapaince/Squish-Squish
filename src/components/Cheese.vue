    ...
    >
      ...
      >
        ...
        <div class="flex flex-row place-items-center gap-1">
          <div v-if="country_thumbnail != undefined">
            <img class="w-4 h-4" :src="`${country_thumbnail}`" />
          </div>
          <div>
            <a class="text-gray-500 text-sm" :href="`${country_link}`">{{ country_name }}</a>
          </div>
        </div>
      </div>
    </div>
